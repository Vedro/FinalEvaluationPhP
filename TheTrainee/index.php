...
// When a get is created with an order and a column value => enter the condition
{
	if($_GET['column'] == 'lastname')
	{
		// if the column is equal to 'lastname' => sort by 'lastname'
		$order = ' ORDER BY lastname';
	}elseif($_GET['column'] == 'firstname')
		{
			// IF NOT AND IF the column is equal to 'firstname' => sort by 'firstname'
			$order = ' ORDER BY firstname';
		}elseif($_GET['column'] == 'birthdate')
			{
				// IF NOT AND IF the column is equal to 'birthdate' => sort by 'birthdate'
				$order = ' ORDER BY birthdate';
			}
	if($_GET['order'] == 'asc')
	{
		// if the order is equal to asc => sort value by ASCENDING way
		$order.= ' ASC';
	}elseif($_GET['order'] == 'desc')
	{
		// if the order is equal to desc => sort value by DESCENDING way
		$order.= ' DESC';
	}
}
...
if(!empty($_POST))
{
	// If the post is not empty, do a foreach to use the values
	foreach($_POST as $key => $value){
		$_POST[$key] = strip_tags(trim($value));
	}
	// checking the length of the firstname | must be superior than 3 char.
	if(strlen($_POST['firstname'] >= 3))
	{
		$errors[] = 'Le prénom doit comporter au moins 3 caractères';		
	}
	// checking the length of the lastname | must be superior than 3 char.
	if(strlen($_POST['lastname'] >= 3))
	{
		$errors[] = 'Le nom doit comporter au moins 3 caractères';
	}
	// checking the email | must be an email format.	
	if(!filter_var($_POST['email'], FILTER_VALIDATE_EMAIL))
	{
		$errors[] = 'L\'adresse email est invalide';
	}
	// checking if the birthdate is not empty
...
// QUERY used for displaying users sorted (and not)
...
<html>
	...
	<body>
		<div class="container">
			...
			<div class="row">
				<?php
				// SUCCESS message for adding a new user
				...
				// ERROR message if adding failed
				if(!empty($errors)){
					echo '<div class="col-md-6 col-md-offset-3">';
				...
				<div class="col-md-7">
					<table class="table">
						...
						<tbody>
						<!-- Use a foreach to display the users from their array -->
